    ...
    namespace ReactApp3.Server.Hubs
    {
        public class ChatHub : Hub
        {
            ...
            public async Task<bool> Login(string username, string password)
            {
                try
                {
                    using (var de = new DirectoryEntry("LDAP://ncseo.ru", username, password))
                    {
                        object nativeObject = de.NativeObject;
                        var user = await _context.GetUserByWindowsUsernameAsync(username);

                        if (user == null)
                        {
                            user = await _context.CreateUserAsync(username, username);
                        ...
                        if (user != null)
                        {
                            ...
                            var onlineUsers = await _context.GetUsersAsync();
                            await Clients.All.SendAsync("UsersList", onlineUsers);
