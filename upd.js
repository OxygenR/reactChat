...
function App() {
    ...
    // Функция для подключения к хабу
    const connectToHub = async () => {
        if (isConnecting.current) return;
        isConnecting.current = true;

        try {
            // Если соединение уже существует и не разорвано, сначала остановим его
            if (connection && connection.state !== signalR.HubConnectionState.Disconnected) {
                await connection.stop();
            }

            const newConnection = new signalR.HubConnectionBuilder()
                .withUrl('http://10.201.252.91:5246/chatHub', {
            ...
            newConnection.on("UserConnected", (user) => {
                setUsers(prev => [...prev, user]);
            });

            newConnection.on("UserDisconnected", (user) => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            const result = await conn.invoke("Login", username, password);

            if (result) {
                setIsAuthenticated(true);

                // Получаем информацию о текущем пользователе
                const user = await conn.invoke("GetCurrentUser");
                setCurrentUser(user);

                // Загружаем пользователей
                const usersList = await conn.invoke("GetUsers");
                setUsers(usersList);
            } else {
    ...
    const reconnect = async () => {
        ...
        try {
            ...
            if (connection && connection.state === signalR.HubConnectionState.Connected) {
                ...
                if (selectedUser) {
                    await loadMessageHistory(selectedUser.userId);
                }
            }
        } catch (error) {
    ...
    useEffect(() => {
        if (connection && isAuthenticated) {
            // Setup additional handlers after authentication
            ...
            connection.on("MessageEdited", (editedMessage) => {
                setMessages(prev => {
                    const index = prev.findIndex(msg => msg.messageId === editedMessage.messageId);
                    if (index !== -1) {
                        const newMessages = [...prev];
                        newMessages[index] = normalizeMessage(editedMessage);
                        return newMessages;
                    }
                    return prev;
                });
